...
def load_and_process_data():
    ...
    # Calculate annual averages and convert to EUR/kWh
    annual_price = price_df.groupby(['Country', 'Year'])['Price (EUR/MWhe)'].mean().reset_index()
    annual_price['Price (EUR/kWhe)'] = annual_price['Price (EUR/MWhe)'] / 1000  # Convert MWh to kWh
    annual_gas = gas_df.groupby('Year')['Gas_Price'].mean().reset_index()
    
    return annual_price, annual_gas

def perform_regression_analysis():
    electricity_df, gas_df = load_and_process_data()
    ...
    for country in all_countries:
        ...
        if len(merged_data) >= 3:  # Need at least 3 data points for regression
            ...
            if len(regression_data) >= 3:
                ...
                y = regression_data['Price (EUR/kWhe)'].values
                ...
                print(f"  Gas price impact: 1 $/MMBtu {gas_impact} electricity price by {abs(model.coef_[0]):.6f} EUR/kWh")
            else:
        ...
        for plot_num in range(num_plots):
            ...
            for i, result in enumerate(results_subset):
                ...
                if not INCLUDE_2022 and 2022 in data['Year'].values:
                    ...
                    scatter1 = axes[i].scatter(data_other['Gas_Price'], data_other['Price (EUR/kWhe)'], 
                    ...
                    scatter2 = axes[i].scatter(data_2022['Gas_Price'], data_2022['Price (EUR/kWhe)'], 
                                             alpha=0.8, s=60, edgecolors='black', color='red', zorder=2,
                                             label='2022 (excluded from fit)')
                else:
                    # Plot all data points normally
                    scatter = axes[i].scatter(data['Gas_Price'], data['Price (EUR/kWhe)'], 
                                            alpha=0.8, s=60, edgecolors='black', color='steelblue', zorder=2)
                
                # Customize plot
                axes[i].set_xlabel('Annual Avg European Natural Gas Price ($/MMBtu)')
                axes[i].set_ylabel('Annual Avg Wholesale Electricity Price (EUR/kWh)')
                ...
                for _, row in data.iterrows():
                    axes[i].annotate(f'{int(row["Year"])}', 
                                   (row['Gas_Price'], row['Price (EUR/kWhe)']),
...
def create_summary_plot(regression_results):
    # Create a summary plot showing gas coefficients and R² values
    ...
    ax1.set_xlabel('Gas Price Coefficient (EUR/kWh per $/MMBtu)')
...
if __name__ == "__main__":
    ...
    if regression_results:
        ...
        print(f"Average gas price coefficient: {avg_gas_coeff:.6f} EUR/kWh per $/MMBtu")
