...
def load_rto_data(rto_name, year):
    ...
    """
    file_path = Path(f'/Users/adessler/Library/Mobile Documents/iCloud~md~obsidian/Documents/BulletVault/plotting code/renewablesSaveMoney/rto_data/{rto_name}_hourly_demand_and_price_{year}.csv')
...
def create_two_panel_plot(results_2023_df, results_2024_df):
    ...
    if not results_2023_df.empty:
        scatter1 = ax1.scatter(results_2023_df['Renewable_Percentage'], 
                              ...
                              s=200, alpha=0.7, color='steelblue', 
...
def create_single_panel_plot_2024(results_2024_df):
    """
    Create a single panel plot for 2024 data only
    ...
    results_2024_df : pd.DataFrame
        DataFrame with 2024 RTO summary statistics
    """
    if results_2024_df.empty:
        print("No 2024 data available for plotting.")
        return
    
    # Create single panel plot
    fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    fig.suptitle('Renewable Energy vs Wholesale Electricity Prices by RTO - 2024', 
                 fontsize=18, fontweight='bold')
    
    # Define colors for each RTO
    rto_color_map = {
        'MISO': '#1f77b4',    # blue
        'ERCOT': '#ff7f0e',   # orange
        'CAISO': '#2ca02c',   # green
    ...
    # Plot 2024 data - use single color
    scatter = ax.scatter(results_2024_df['Renewable_Percentage'], 
                        results_2024_df['Weighted_Average_Price'],
                        s=300, alpha=0.7, color='steelblue',
    ...
    # Customize the plot
    ax.set_xlabel('Renewable Energy Percentage (Wind + Solar) (%)', fontsize=14)
    ax.set_ylabel('Load-Weighted Average Price ($/MWh)', fontsize=14)
    ax.grid(True, alpha=0.3)
    ax.tick_params(axis='both', which='major', labelsize=12)
    
    # Add some padding to the axes
    x_margin = (results_2024_df['Renewable_Percentage'].max() - results_2024_df['Renewable_Percentage'].min()) * 0.1
    y_margin = (results_2024_df['Weighted_Average_Price'].max() - results_2024_df['Weighted_Average_Price'].min()) * 0.1
    
    ax.set_xlim(results_2024_df['Renewable_Percentage'].min() - x_margin, 
                results_2024_df['Renewable_Percentage'].max() + x_margin)
    ax.set_ylim(results_2024_df['Weighted_Average_Price'].min() - y_margin, 
                results_2024_df['Weighted_Average_Price'].max() + y_margin)
    
    # Adjust layout and show
    plt.tight_layout()
    # plt.savefig('renewable_vs_price_2024_single_panel.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # Create single panel plot for 2024 only
    print("\nCreating single panel plot for 2024 data...")
    create_single_panel_plot_2024(results_2024_df)
